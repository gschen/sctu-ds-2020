...
# 	pass
class  Gameperson():
    def _init_(self,name,blood,attack,violence,dodg):
        self.name=name 
        self.blood=blood
        self.attack=attack
...
a=Gameperson('马',1000,100,0.4,0.6)
b=Gameperson('洁',2000,50,0.1,0.1)
a.information()
b.information()   
	...
	def playGame(self):
		...
		while self.player1.blood>0 and self.player2.blood>0:
			...
			if self.player2.blood<=0:
				self.player2.blood=0
			print('第%d轮攻击:' % count)
			print('马:剩余血量: %d' % self.player1.blood)
			print('洁:剩余血量: %d' % self.player2.blood)
		if self.player1.blood == 0:
			print('经过%d轮攻击,洁成功击杀马,成为本局胜利者.' % count)
		else:
			print('经过%d轮攻击,马成功击杀洁,成为本剧胜利者.' % count)
a=Gameperson('马',1000,100,0.4,0.6)
b=Gameperson('洁',2000,50,0.1,0.1)
c=bing()
c.playGame()
