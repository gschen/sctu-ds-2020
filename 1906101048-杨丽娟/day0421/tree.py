class Node(object):
    def __init__(self,item):
        ...
        self.right = None
    def __str__(self):
        return str(self.item) #print一个Node类打印__str__的返回值
...
class Tree(object):
    ...
    def add(self,item):
        node = Node(item)
        if self.root is None: # 如果二叉树为空，那么生成的二叉树最终为新插入的节点
            self.root = node
        else:
            q = [self.root] # 将q列表添加二叉树的根节点
            while True:
                pop_node = q.pop(0)
                if pop_node.left is None: # 如果左子树为空，则将添加到左子树
                    pop_node.left = node
                    return
                elif pop_node.right is None: # 如果右子树为空，则将点添加到右子树
                    pop_node.right = node
                    return
                else:
