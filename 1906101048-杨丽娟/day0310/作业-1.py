...
class Person(object):
    def __init__(self,name,blood,attack,crit,dodge):
        ...
        self.dodge=dodge
    def personinfo(self):
        print(('名称:%s,血量:%s,攻击力:%s,暴击率:%s,闪避率:%s')%(self.name,self.blood,self.attack,self.crit,self.dodge))
...
class Game():
    ...
    def playGame(self):
        ...
        if self.player1.blood == 0:
            print('经过%d轮攻击，S成功击杀Y，成为本局胜利者!' % n)
        else:
            print('经过%d轮攻击，Y成功击杀S，成为本局胜利者!' % n)
a=Person('Y',20000,1000,0.4,0.3)
b=Person('S',20000,1100,0.3,0.2)
yi=Game()
yi.playGame()