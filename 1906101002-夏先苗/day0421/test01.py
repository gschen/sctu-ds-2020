class Node():
    def __init__(self,item):
        ...
        self.right=None
    def __str__(self):
        return str(self.item)#print 一个Node类是会打印__str__的返回值
class Tree(object):
    def __init__(self):
        self.root=Node('root')#根节点定义为root 永不删除
    def add(self,item):
        node=Node(item)
        if self.root is None:#如果二叉树为空，那么生成的二叉树最终为新插入树
            self.root=node
        else:
            q=[self.root]#将q列表添加二叉树的根节点
            while True:
                pop_node=q.pop(0)
                if pop_node.left is None:#如果左子树为空，则将该节点添加到
                    pop_node.left=node
                    return
                elif pop_node.right is None:#右子树为空则将点添加到右子树中
                    pop_node.right=node
                    return 
                else:
        ...
        if parent:
            ...
            if del_node.left is None:
                ...
                if tmp_next.left is None:
                    ...
                    tmp_next.left=del_node.leeft
                    tmp_next.right=del_node
                if parent.left.item==item:
