...
class Node(object):
    def __init__(self,item):
        ...
        self.right=None
    def __str__(self):
        return str(self.item) # 打印一个Node类时会打印__str__的返回值

class Tree(object):
    def __init__(self):
        self.root=Node('root') # 定义根节点为root，永不删除
        self.size=1
    def add(self,item):
        self.size+=1
        node=Node(item)
        if self.root is None: # 如果二叉树为空，那么生成的二叉树最终为新插入的结点
            self.root=node
        else:
            q=[self.root]
            while True:
                ...
                if pop_node.left is None: # 如果左子树为空，则将该点添加到左子树
                    pop_node.left=node
                    return
                elif pop_node.right is None: #如果右子树为空则将该点添加到右子树
                    pop_node.right=node
                    return
                else:
