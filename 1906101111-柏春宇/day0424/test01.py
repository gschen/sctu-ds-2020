...
class Tree(object):
    def __init__(self):
        self.root=Node('root')#根节点定义root，永不删除
    def add(self,item):
        node=Node(item)
        if self.root is None:#如果二叉树为空，那么生成的二叉树最终为新插入的点
            ...
            while True:
                ...
                if pop_node.left is None:#如果左子树为空，则将该点添加到左子树
                    pop_node.left=node
                    return
                elif pop_node.right is None:#如果右子树为空，则将该点添加到右子树
                    pop_node.right=node
                    return
                else:
        ...
        if parent:
            ...
            if del_node.left is None:
                ...
                if tmp_next.left is None:
                    ...
                    tmp_per.left=tmp_next.right
                    tmp_next.left=del_node.left
                    tmp_next.right=del_node.left
                if parent.left.item==item:
