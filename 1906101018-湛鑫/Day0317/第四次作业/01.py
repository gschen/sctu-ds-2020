...
a=Player('火星人',1000,100,0.5,0.5)
b=Player('BOSS',2000,200,0.7,0.1)
a.information()
b.information()
	...
	def playGame(self):
		...
		while self.player1.blood>0 and self.player2.blood>0:
			...
			if self.player2.blood<=0:
				self.player2.blood=0
			print('第%d轮攻击:' % count)
			print('火星人:剩余血量: %d' % self.player1.blood)
			print('BOSS:剩余血量: %d' % self.player2.blood)
		if self.player1.blood == 0:
			print('经过%d轮攻击,BOSS成功击杀火星人,成为本局胜利者.' % count)
		else:
			print('经过%d轮攻击,火星人头成功击杀BOSS,成为本剧胜利者.' % count)
a=Player('大头',1000,100,0.5,0.5)
b=Player('BOSS',2000,200,0.7,0.1)
c=zhan()
c.playGame()
