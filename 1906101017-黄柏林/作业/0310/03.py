...
a=Player('卢本伟',3000,999,0.9,0.9)
b=Player('五五开',3000,100,0.2,0.2)
a.role()
b.role()
class Game():
    def __init__(self):
    ...
    def playerGame(self):
        ...
        while self.player1.blood>0 and self.player2.blood>0:
            ...
            if self.player2.blood<=0:
                self.player2.blood=0
            print('第%d轮攻击:'% count)
            print('卢本伟:剩余血量:%d'% self.player1.blood)
            print('五五开:剩余血量:%d'% self.player2.blood)
        if self.player1.blood==0:
            print('经过%d轮攻击,五五开成功击杀卢本伟,成为本局胜利者.'% count)
        else:
            print('经过%d轮攻击，卢本伟成功击杀五五开,成为本局胜利者.'% count)
a=Player('卢本伟',3000,999,0.9,0.9)
b=Player('五五开',3000,100,0.2,0.2)
c=Game()
c.playerGame()