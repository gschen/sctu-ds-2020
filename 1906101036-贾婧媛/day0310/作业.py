...
a=Gameperson('大头',1000,100,0.4,0.6)
b=Gameperson('BOSS',2000,50,0.1,0.1)
a.information()
b.information()
	...
	def playGame(self):
		...
		while self.player1.blood>0 and self.player2.blood>0:
			...
			if self.player2.blood<=0:
				self.player2.blood=0
			print('第%d轮攻击:' % count)
			print('大头:剩余血量: %d' % self.player1.blood)
			print('BOSS:剩余血量: %d' % self.player2.blood)
		if self.player1.blood == 0:
			print('经过%d轮攻击,BOSS成功击杀大头,成为本局胜利者.' % count)
		else:
			print('经过%d轮攻击,大头成功击杀BOSS,成为本剧胜利者.' % count)
a=Gameperson('大头',1000,100,0.4,0.6)
b=Gameperson('BOSS',2000,50,0.1,0.1)
c=Jia()
c.playGame()
